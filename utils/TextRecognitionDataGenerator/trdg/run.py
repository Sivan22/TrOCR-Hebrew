...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="?",
        help="The language to use, should be he (Hebrew), fr (French), en (English), es (Spanish), de (German), ar (Arabic), cn (Chinese), ja (Japanese) or hi (Hindi)",
        default="en",
    )
    parser.add_argument(
        "-c",
...
def main():
    ...
    if args.language == "ar" or args.language == "he" or args.language == "he_hw"  :
    ...
    if args.name_format == 2:
        ...
        with open(
            os.path.join(args.output_dir, "metadata.csv"), "w", encoding="utf8"
        ) as f:
            for i in range(string_count):
                file_name = str(i) + "." + args.extension
                label = strings[i]
                if args.language == "he" or args.language == "he_hw":
                    label= label[::-1]
                if args.space_width == 0:
                    label = label.replace(" ", "")
                f.write('{},"{}"\n'.format(file_name, label.replace("'","\'")))
